/************************************************************************
* ------ - 使用GamebabyRockSun CSDN D3D12基础教程项目作为基本框架--------
* FileName : D3D12_Basis_Main.cpp
* Version  : v1.0
* Description : 将所有D3D12的操作放到基本的WinMain函数中运行，便于直观理解D3D12框架
* Author   : Ssi
* Created  : 2020/10/5
* History  : 以下开始记录修改历史
* 1.Date   : 2020/10/5
* Author   : Ssi
* Modify   : 修改了shader.hlsl路径
			 修改了显卡适配器枚举索引
* **********************************************************************/
...
#include <windows.h>
#include <tchar.h>
// 添加WRL支持 方便使用COM，使用WRL中的ComPtr对D3D12的COM接口变量进行了模板参数实例化封装，因此无需在使用完COM接口后将其手动release掉
#include <wrl.h>
#include <strsafe.h>
#include <dxgi1_6.h>
...
struct GRS_VERTEX
{
	XMFLOAT4 m_vtPos;   // XMFLOAT3/4来自DirectXMath库，等价于float[3]/[4]
	XMFLOAT4 m_vtColor;
};
...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR    lpCmdLine, int nCmdShow)
{
	...
	try
	{
		...
		// 创建DXGI Factory对象，用于创建适配器、3D设备等对象接口
		{
			GRS_THROW_IF_FAILED(CreateDXGIFactory2(nDXGIFactoryFlags, IID_PPV_ARGS(&pIDXGIFactory5))); // 接口和函数后面的数字表示其版本号
			// 关闭ALT+ENTER键切换全屏的功能，因为我们没有实现OnSize处理，所以先关闭
			GRS_THROW_IF_FAILED(pIDXGIFactory5->MakeWindowAssociation(hWnd, DXGI_MWA_NO_ALT_ENTER));

		}

		// 枚举适配器，并选择合适的显示适配器（显卡）来创建3D设备对象；也为将来使用多显卡渲染实例的需要作准备
		{
			...
			for (UINT adapterIndex = 0; DXGI_ERROR_NOT_FOUND != pIDXGIFactory5->EnumAdapters1(adapterIndex, &pIAdapter1); ++adapterIndex)
			{
				...
				if (stAdapterDesc.Flags & DXGI_ADAPTER_FLAG_SOFTWARE) // 日后可以在此处设置断点查看是否选中想用的显卡
				{// 跳过软件虚拟适配器设备
					continue;
				}
		...
		// 创建直接命令队列/本质上可以执行几乎所有命令队列的命令，共有四种命令队列（3D图形、复制、计算、视频编码），相当于四种分工不同的引擎
		{
		...
		// 创建交换链
		{
			...
			GRS_THROW_IF_FAILED(pISwapChain1.As(&pISwapChain3)); // As函数：用pISwapChain1的参数来填充pISwapChain3
			...
			//创建RTV(渲染目标视图)描述符堆(这里堆的含义应当理解为数组或者固定大小元素的固定大小显存池)
			{
				...
				stRTVHeapDesc.NumDescriptors = nFrameBackBufCount; // 指定数组元素的个数（这里可以将描述符堆理解为数组）
				...
				//得到每个描述符元素的大小
				nRTVDescriptorSize = pID3D12Device4->GetDescriptorHandleIncrementSize( // 字节大小
		...
		// 创建顶点缓冲
		{
			...
			D3D12_HEAP_PROPERTIES stHeapProp = { D3D12_HEAP_TYPE_UPLOAD }; // UPLOAD是指从CPU内存上传到显存
